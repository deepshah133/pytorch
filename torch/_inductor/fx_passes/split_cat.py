...
def normalize_reshape_default(match: Match, *args, **kwargs):
    ...
    reshape_arg_list = list(reshape_node.args)
    reshape_input = reshape_arg_list[0]
    reshape_shape = reshape_arg_list[1:]

    with match.graph.inserting_after(reshape_node):
        new_reshape_node = match.graph.call_function(
            torch.reshape, args=(reshape_input, tuple(reshape_shape)), kwargs={}
        )
